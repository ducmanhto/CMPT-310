...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if(self.turnCostOn == True):
            cost = cost + 0.5 * self.computeTurnCost(curNode.action, action)
    ...
    def findMinManhattanDist(self, pos):
        """Find the minimum distance between position pos and any of the dirty rooms.
        Dirty rooms are maintained in self.env.dirtyRooms."""
        ...
        if self.env.dirtyRooms:
            ...
            for room in self.env.dirtyRooms:
                dX, dY = room
                curMin = min(curMin, abs(dX - aX) + abs(dY - aY))
