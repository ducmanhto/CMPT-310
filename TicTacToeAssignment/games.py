...
def minmax_cutoff(game, state):
    ...
    def max_value(state, d):
        ...
        if game.terminal_test(state) or d == game.d:
    ...
    if player == 'O':
        return min(game.actions(state), key=lambda a: max_value(game.result(state, a), 1), default=None)
    else:
...
def alpha_beta(game, state):
    ...
    if player == 'X':
        best_score = np.inf
        for a in game.actions(state):
            v = min_value(game.result(state, a), -np.inf, np.inf)
            if v < best_score:
                best_score = v
                best_action = a
    else:
        best_score = -np.inf
        for a in game.actions(state):
            v = max_value(game.result(state, a), -np.inf, np.inf)
            if v > best_score:
                best_score = v
                best_action = a
...
def alpha_beta_cutoff(game, state):
    ...
    if player == 'O':
        best_score = np.inf
        for a in game.actions(state):
            v = max_value(game.result(state, a), alpha, beta, 1)
            if v < best_score:
                best_score = v
                best_action = a
            beta = min(beta, v)
        ...
        best_score = -np.inf
        for a in game.actions(state):
            v = min_value(game.result(state, a), alpha, beta, 1)
            if v > best_score:
                best_score = v
                best_action = a
            alpha = max(alpha, v)
...
def alpha_beta_player(game, state):
    ...
    #print("Your code goes here -10pt")
    depth = 1
    best_move = random_player(game, state)
   ...
   #print("Your code goes here -10pt")
    ...
    while time.perf_counter() < end:
        ...
        if move is not None:
            best_move = move
        depth += 1
        if depth > game.maxDepth:  # If depth exceeds the maximum depth, stop
            break
...
class TicTacToe(Game):
    ...
    def eval1(self, state):
        ...
        if len(state.moves) <= self.k / 2:
            return 0
        """
        def possible_k_complete(board, player, k):
            match_count = 0
            for move in board:
                if board[move] == player:
                    if self.k_in_row(board, move, player, (0, 1), k - 1):
                        match_count += 1
                    if self.k_in_row(board, move, player, (1, 0), k - 1):
                        match_count += 1
                    if self.k_in_row(board, move, player, (1, -1), k - 1):
                        match_count += 1
                    if self.k_in_row(board, move, player, (1, 1), k - 1):
                        match_count += 1
            return match_count
        ...
        if check_win(state.board, 'X'):
            return float('inf')  # X wins
        elif check_win(state.board, 'O'):
            return float('-inf')  # O wins

        x_count = possible_k_complete(state.board, 'X', self.k)
        o_count = possible_k_complete(state.board, 'O', self.k)
